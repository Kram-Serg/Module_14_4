...
async def get_buying_list(message):
    await message.answer(f'Название: {product[0][0]} | Описание: {product[0][1]}'
                         f' | Цена: {product[0][2]}')
    with open('files/1.png', 'rb') as img:
        await message.answer_photo(img)
    await message.answer(f'Название: {product[1][0]} | Описание: {product[1][1]}'
                         f' | Цена: {product[1][2]}')
    with open('files/2.png', 'rb') as img:
        await message.answer_photo(img)
    await message.answer(f'Название: {product[2][0]} | Описание: {product[2][1]}'
                         f' | Цена: {product[2][2]}')
    with open('files/3.png', 'rb') as img:
        await message.answer_photo(img)
    await message.answer(f'Название: {product[3][0]} | Описание: {product[3][1]}'
                         f' | Цена: {product[3][2]}')
